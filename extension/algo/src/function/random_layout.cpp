...
static constexpr double DEFAULT_HEIGHT = 1000.0;

// Bind data for random layout - uses GDS pattern for per-node output
struct RandomLayoutBindData final : public GDSBindData {
    RandomLayoutBindData(expression_vector columns, NativeGraphEntry graphEntry,
    ...
    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<RandomLayoutBindData>(*this);
    }
};
...
class LayoutXValues {
public:
    LayoutXValues(table_id_map_t<offset_t> maxOffsetMap, storage::MemoryManager* mm) {
...
class LayoutYValues {
public:
    LayoutYValues(table_id_map_t<offset_t> maxOffsetMap, storage::MemoryManager* mm) {
        for (const auto& [tableID, maxOffset] : maxOffsetMap) {
            valueMap.allocate(tableID, maxOffset, mm);
            pinTable(tableID);
            for (auto i = 0u; i < maxOffset; ++i) {
...
class LayoutResultVertexCompute : public GDSResultVertexCompute {
public:
    LayoutResultVertexCompute(storage::MemoryManager* mm, GDSFuncSharedState* sharedState,
        LayoutXValues& xValues, LayoutYValues& yValues)
    ...
    std::unique_ptr<VertexCompute> copy() override {
        return std::make_unique<LayoutResultVertexCompute>(mm, sharedState, xValues, yValues);
    }

private:
    LayoutXValues& xValues;
    LayoutYValues& yValues;
...
static offset_t tableFunc(const TableFuncInput& input, TableFuncOutput&) {
    ...
    auto xValues = LayoutXValues(maxOffsetMap, mm);
    auto yValues = LayoutYValues(maxOffsetMap, mm);

    // Compute random positions for each node
    for (const auto& [tableID, maxOffset] : maxOffsetMap) {
        xValues.pinTable(tableID);
        yValues.pinTable(tableID);

        for (offset_t i = 0; i < maxOffset; ++i) {
            xValues.setValue(i, xDist(rng));
            yValues.setValue(i, yDist(rng));
        }
    }

    // Output results
    auto outputVC = std::make_unique<LayoutResultVertexCompute>(mm, sharedState, xValues, yValues);
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    return std::make_unique<RandomLayoutBindData>(std::move(columns), std::move(graphEntry),
        nodeOutput);
}

function_set RandomLayoutFunction::getFunctionSet() {
    function_set result;
    auto func = std::make_unique<TableFunction>(RandomLayoutFunction::name,
